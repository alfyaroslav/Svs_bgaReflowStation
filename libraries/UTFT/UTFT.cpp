...
void UTFT::InitLCD(byte orientation, SPIClass &wspi)
{
	orient=orientation;
	_hw_special_init();
...
void UTFT::printChar(byte c, int x, int y)
{
	...
	if (!_transparent)
	{
		if (orient==PORTRAIT)
		{
			...
			if (nextaltfont) {
				if (shiftRU == 0) temp=((c-altfont.offset)*(jn*cfont.y_size))+4;
					else temp=((c-altfont.numchars)*(jn*cfont.y_size))+4+shiftRU;
			}
			else
			{ 
				temp=((c-cfont.offset)*(jn*cfont.y_size))+4;	// jn=cfont.x_size/8;
			} 
			// temp=((c-cfont.offset)*(jn*cfont.y_size))+4;
            
			for(j=0;j<(jn*cfont.y_size);j++)
			{
				if (nextaltfont) ch=pgm_read_byte(&altfont.font[temp]);
						else ch=pgm_read_byte(&cfont.font[temp]);
		...
		else
		{
			...
			for(j=0;j<(jn*cfont.y_size);j+=jn)
			{
				setXY(x,y+(j/jn),x+cfont.x_size-1,y+(j/jn)); 
                
				for (int zz=jn-1; zz>=0; zz--)
				{
					if (nextaltfont) ch=pgm_read_byte(&altfont.font[temp+zz]);
						else ch=pgm_read_byte(&cfont.font[temp+zz]);
...
void UTFT::print(char *st, int x, int y, int deg)
{
	...
	//Serial.print(stl); Serial.print(" "); Serial.println(stlr);

	if (orient==PORTRAIT)
	{
	  if (x==RIGHT)
		    x=(disp_x_size+1)-(stlr*cfont.x_size);
	  if (x==CENTER)
		    x=((disp_x_size+1)-(stlr*cfont.x_size))/2;
	}
	else
	{
	  if (x==RIGHT)
		    x=(disp_y_size+1)-(stlr*cfont.x_size);
	  if (x==CENTER)
		    x=((disp_y_size+1)-(stlr*cfont.x_size))/2;
	}
	...
	for (i=0; i<stl; i++) {
		...
		ch = *st;
		if (setaltfont && (ch==0xD0 || ch==0xD1))  {
			nextaltfont = true;
			if (ch==0xD1) shiftRU = 48*32;
				else shiftRU = 0;
			i++;
			d++;
			ch = *st++;	
		...
		if (deg==0)			
			printChar(*st++, x + ((i-d)*(cfont.x_size)), y);
		else
			rotateChar(*st++, x, y, i-d, deg);
	}
}
...
void UTFT::setFont(uint8_t* font)
{
	...
	cfont.offset=fontbyte(2);	//смещение - (32)
	cfont.numchars=fontbyte(3);
	jn=cfont.x_size/8;
	setaltfont = false;
}

void UTFT::setFontAlt(uint8_t* font) 
{
	altfont.font=font;   //BFontRu  количество символов в шрифте (64) D0 - 48, D1 - 16
	altfont.x_size=altfontbyte(0);
	altfont.y_size=altfontbyte(1);
	if (altfont.x_size == cfont.x_size && altfont.y_size == cfont.y_size) {
		altfont.offset=altfontbyte(2);		//смещение при D0 - (x90)
		altfont.numchars=altfontbyte(3);	//смещение при D1 - (x80)
		setaltfont = true;
	}
}
